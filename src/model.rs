...
pub trait Model {
    ...
    fn step(&self, state: &Self::State, input: &Self::Input, output: &Self::Output) -> Option<Self::State>;
    ...
    /// independent.
    fn partition(&self, _history: &[Operation<Self::Input, Self::Output>]) -> Option<Vec<Vec<usize>>> {
        None
    }

    /// Optionally partition an event-based history into independent sub-histories.
    fn partition_events(&self, _history: &[Event<Self::Input, Self::Output>]) -> Option<Vec<Vec<usize>>> {
        None
    }
}
