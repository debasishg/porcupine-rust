...
// ---------------------------------------------------------------------------
// Parallel entry-point MBT tests  (requires both quint-mbt AND parallel)
// ---------------------------------------------------------------------------

/// Verify that `check_operations_parallel` returns the same result as the
/// Quint formal model on the example history.
///
/// This pins the parallel rayon path against the formal model, confirming
/// INV-LIN-01 (Soundness) and INV-LIN-02 (Completeness) hold on the
/// parallel code path.
#[cfg(feature = "parallel")]
#[test]
fn mbt_parallel_ops_matches_quint_trace() {
    let trace_path = std::env::temp_dir().join("porcupine_mbt_par_ops_trace.itf.json");
    let status = Command::new("quint")
        .args([
            "run",
            "tla/Porcupine.qnt",
        ...
        .status()
        .expect("Failed to run `quint` — is it installed?");
    ...
    assert_eq!(
        ...
        "MBT parallel-ops mismatch: Quint says {:?}, check_operations_parallel says {:?}.",
        quint_result, par_result
    );
}

/// Verify that `check_events_parallel` returns the same result as the Quint
/// formal model on the example history expressed as an event stream.
///
/// Exercises INV-LIN-01 (Soundness) and INV-LIN-02 (Completeness) on both
/// the event pipeline and the rayon parallel path simultaneously.
#[cfg(feature = "parallel")]
#[test]
fn mbt_parallel_events_matches_quint_trace() {
    let trace_path = std::env::temp_dir().join("porcupine_mbt_par_events_trace.itf.json");
    let status = Command::new("quint")
        .args([
            "run",
            "tla/Porcupine.qnt",
    ...
    assert_eq!(
        par_result, quint_result,
        "MBT parallel-events mismatch: Quint says {:?}, check_events_parallel says {:?}.",
        quint_result, par_result
    );
}
